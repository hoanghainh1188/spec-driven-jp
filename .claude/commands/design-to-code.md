...
11. **[TỰ CHẠY] Gọi subagent `glossary-steward`** (qua Task tool) để đối chiếu tên biến/field nghiệp
    ...
    vụ mới → ghi chú đề xuất mở **PR glossary riêng** cho steward duyệt (KHÔNG tự thêm vào glossary).
...
13. **[DỪNG] Test gate** — Format đã tự chạy qua PostToolUse hook (`.claude/hooks/format.sh`) sau mỗi
    ...
    (hoặc lệnh tương đương của dự án). **Bắt buộc xanh** mới đi tiếp; nếu đỏ, báo lỗi và dừng —
    không deploy khi test/build còn fail. (Tuỳ chọn: chạy `git diff --exit-code` sau format để chắc
    code đã format sạch trước commit.)
